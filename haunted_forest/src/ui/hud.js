export function initHUD() {
  ...
  if (!document.getElementById('hud-style')) {
    ...
    style.textContent = `
      ...
      .hud-card{ position: fixed; left:16px; top:16px; z-index: 10000; min-width: 380px;
        color: var(--hud-fg); background: var(--hud-bg); border-radius: 14px;
        box-shadow: 0 14px 32px #0008, inset 0 1px 0 #fff1; padding: 14px 16px;
        backdrop-filter: blur(6px); font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, "Helvetica Neue", Arial, "Noto Sans", "Liberation Sans", sans-serif; font-size: 14px; }
      .hud-row{ display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 10px; margin: 10px 0; }
      .hud-label{ opacity: .95; display:flex; align-items:center; gap:8px; white-space: nowrap; }
      .hud-icon{ width: 18px; height: 18px; display:inline-flex; align-items:center; justify-content:center; filter: drop-shadow(0 1px 0 #0006); }
      .hud-value{ color: var(--hud-muted); font-variant-numeric: tabular-nums; min-width: 44px; text-align:right; }
      .meter{ position: relative; height: 12px; border-radius: 999px; overflow: hidden;
        background: linear-gradient(#0000, #0002), var(--hud-track); box-shadow: inset 0 1px 2px #0008, inset 0 0 0 1px #0006; }
      .meter.slim{ height: 10px; }
      .meter > .fill{ position:absolute; inset:0; width:0%;
        background: linear-gradient(90deg, #16d6a3, #19b88a);
        box-shadow: 0 0 10px #16d6a380; transition: width .12s ease; }
      .meter.heat > .fill{ background: var(--heat-color, #20b8ff);
      ...
      .meter.heat.over > .fill{ background: var(--hud-danger); box-shadow: 0 0 16px #ff6b6b88; animation: pulseRed .8s ease-in-out infinite; }
      .hud-badge{ padding: 2px 8px; border-radius: 999px; font-weight: 700; font-size: 12px; letter-spacing:.2px;
        background: #33415566; color: #cbd5e1; text-transform: lowercase; white-space: nowrap; }
      .badge-inactive{ background:#47556944; color:#cbd5e1; }

      .hud-rightstack{ display:flex; align-items:center; gap:8px; justify-content:flex-end; flex-wrap: wrap; min-width: 220px; }

      /* Sanctuary badge palette */
      ...
      .sanct-safe{ display:none !important; } /* <-- sempre nascosto */
    `;
    /* === Offscreen Indicators CSS === */
    style.textContent += `
      #indicator-layer{
      ...
      .indic .arrow{
        ...
        border-bottom:16px solid var(--col, #93c5fd); /* punta giù: ruotiamo il wrapper */
      }
      .indic.info  { --col: #93c5fd; } /* azzurrino */
      .indic.warn  { --col: #fbbf24; } /* ambra     */
      .indic.danger{ --col: #ff6b6b; } /* rosso     */
    `;
  ...
  root.innerHTML = `
    <div class="hud-row">
      <div class="hud-label"><span class="hud-icon">❤️</span><span>Health</span></div>
      <div class="meter" id="hud-health-meter"><div class="fill" id="hud-health-fill"></div></div>
      <div class="hud-value" id="hud-health-text">100</div>
    </div>

    <div class="hud-row">
      <div class="hud-label"><span class="hud-icon">🔦</span><span>Beam&nbsp;Heat</span></div>
      <div class="meter heat" id="hud-heat-meter"><div class="fill" id="hud-heat-fill"></div></div>
      <div class="hud-value" id="hud-heat-text">0%</div>
    </div>

    <div class="hud-row">
      <div class="hud-label"><span class="hud-icon">⭐</span><span>Score</span></div>
      <div class="meter" id="hud-score-meter"><div class="fill" id="hud-score-fill" style="opacity:.25"></div></div>
      <div class="hud-value" id="hud-score-text">0</div>
    </div>

    <!-- Sanctuary row -->
    <div class="hud-row">
      <div class="hud-label"><span class="hud-icon">🛕</span><span>Sanctuary</span></div>
      <div>
        <div class="meter slim" id="hud-sanct-meter" title="progress"><div class="fill" id="hud-sanct-fill"></div></div>
        <div style="margin-top:6px; display:flex; gap:8px; align-items:center;">
          <span id="hud-sanct-state" class="hud-badge sanct-idle">idle</span>
          <span id="hud-sanct-safe"  class="hud-badge sanct-safe">SAFE</span>
    ...
    <div class="hud-row">
      <div class="hud-label"><span class="hud-icon">👻</span><span>Ghost</span></div>
    ...
    <!-- Spawner row -->
    <div class="hud-row">
      <div class="hud-label"><span class="hud-icon">🧲</span><span>Spawner</span></div>
      <div class="hud-rightstack">
  ...
  /* === Indicator overlay & pool === */
  const indicLayer = document.createElement('div');
  indicLayer.id = 'indicator-layer';
  document.body.appendChild(indicLayer);

  const indicPool = []; // [{el, arrow, busy}]
  function _getIndicEl(){
    for (const it of indicPool) {
      if (!it.busy){ it.busy = true; it.el.style.display='block'; return it; }
    }
    const el = document.createElement('div');
    el.className = 'indic info';
    const arrow = document.createElement('div');
    arrow.className = 'arrow';
    el.appendChild(arrow);
    indicLayer.appendChild(el);
    const rec = { el, arrow, busy:true };
    indicPool.push(rec);
    return rec;
  }
  ...
  // aggiornamento Sanctuary (senza SAFE)
  ...
  // items: [{ x, y, ang, severity:'info'|'warn'|'danger', alpha:0..1, scale:0.6..1.2 }]
  function setIndicators(items = []){
    ...
    for (const it of items){
      ...
      // arrow default punta in GIÙ → ruotiamo di +90° per allineare al vettore (x,y)
      const ang = (it.ang ?? 0) + Math.PI * 0.5;
      rec.el.style.opacity = a.toFixed(3);
      rec.el.style.transform =
        `translate(${Math.round(it.x)}px, ${Math.round(it.y)}px) rotate(${ang}rad) scale(${s})`;
    }
  }
