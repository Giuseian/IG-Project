// // // // main.js — Pines + FBM Fog + HUD + Beam Heat + Ghost SPAWNER (graduale) + WASD kinematic
...
// import { WASDController } from './systems/WASDController.js';   // << NEW
...
//   controls.enablePan = false;         // << consigliato con WASD
...
//   // ---------- WASD Controller (kinematic) ----------
//   const getGroundY = (x, z) => 0.0; // terreno piatto per ora
//   playerCtl = new WASDController({
//     camera,
//     controls,
//     getGroundY,
//     eyeHeight: 20,        // mantieni la camera a 20 sopra il terreno
//     speed: 50,            // tuning velocità base
...
//   // 6) HUD: ghost (primo attivo come rappresentativo) + spawner stats
...
//   // 7) controls (rotazione mouse) + render
//   controls.update();
...
// // // main.js — Pines + FBM Fog (safe) + HUD + Ghost SPAWNER + WASD kinematic  - buono starting point per la nebbia 
import * as THREE from 'https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.module.js';
import { OrbitControls } from 'https://cdn.jsdelivr.net/npm/three@0.160.0/examples/jsm/controls/OrbitControls.js';
...
THREE.ShaderChunk.fog_fragment = `
  ...
  float fogFactor = clamp( mix(heightFog, heightFog + baseExp*0.6, 0.7) * mix(n, 1.0, clamp((dist - 5000.0)/5000.0, 0.0, 1.0)), 0.0, 1.0);
  gl_FragColor.rgb = mix(gl_FragColor.rgb, fogColor, fogFactor);
#endif
`;
...
async function init(){
  scene = new THREE.Scene();
  scene.background = new THREE.Color(0x87a0c0);
  scene.fog = new THREE.FogExp2(0xDFE9F3, 1.6e-4); // più densa
  ...
  // Beam (SpotLight) – rimane spento, serve solo come luce scenica quando attivo
  beam = new THREE.SpotLight(0xcff2ff, 0, 60, THREE.MathUtils.degToRad(12), 0.35, 1.0);
  beam.visible = false;
  camera.add(beam);
  scene.add(camera);
  ...
  // Input beam + toggles (non indispensabile ora, ma lascio i comandi)
  addEventListener('mousedown', (e)=>{ if (e.button === 0 && !player.overheated) player.beamOn = true; });
  addEventListener('mouseup',   (e)=>{ if (e.button === 0) player.beamOn = false; });
  addEventListener('keydown',   (e)=>{
    const k = e.key;
  ...
  // ---------------- GHOST SPAWNER ----------------
  // Target = la TUA posizione (non il focus davanti).
  // Lo spawn ring resta attorno a te ma a raggio grande, così sono "sparpagliati".
  ...
  spawner = new GhostSpawner({
    ...
    maxAlive: 5,
    spawnInterval: 1.2,
    // anello grande = non percepisci il "cerchio" vicino
    minR: 140,
    maxR: 260,
    minPlayerDist: 60,
    minSeparation: 40,
    ...
    ghostOpts: {
      url: '/assets/models/ghost/ghost.glb',
      targetHeight: env.pineTypicalHeight * 0.22, // << molto più piccoli
      scaleJitter: 0.25,
      opacityBody: 0.78,
      speed: 5.2,
      burstMultiplier: 1.5,
      keepDistance: 0.2,
      arriveRadius: 1.4,
      yawRateDeg: 540, // reagiscono velocemente ai cambi camera
      swoop: { far: 100, near: 45, hLow: 1.6, hHigh: 5.8, yLerp: 3.2 }
    },
    protectSeconds: 1.0
  });
  await spawner.init();
...
function animate(){
  ...
  if (typeof hud.setDebug === 'function') {
    hud.setDebug({ state: g?.state ?? 'inactive', threshold: thr, exposure: g?.exposure || 0, dist, spawner: spStats });
  }
