...
// // // // // // main.js — Pines + FBM Fog (safe) + HUD + Ghost SPAWNER + WASD (FPS) + BeamSystem (GIMBAL) + SANCTUARIES + pointer lock 
...
//   // Pointer Lock: click sul canvas per abilitare mouse-look
...
//     `shaders:${_fogShaders.size}  |  anim:${animateFog?'ON':'OFF'}${spLine}${beamInfo}\n` +
//     `Heat: ${player.overheated?'<span style="color:#ff6b6b">'+heatPct+'%</span>':heatPct+'%'}  ` +
//     `| Beam: ${beamState}  (Pointer Lock: click canvas | RMB: AIM | F: power | P:spawn, V:antiPopIn, C:cleanse all, X:nearest, ,/. angle, 9/0 range, Q/E snap, Shift+Q 180°)`;
// }
