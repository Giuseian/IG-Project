...
// src/main_ghost_only.js
import * as THREE from 'https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.module.js';
import { OrbitControls } from 'https://cdn.jsdelivr.net/npm/three@0.160.0/examples/jsm/controls/OrbitControls.js';
import { Ghost } from './entities/Ghost.js';

let scene, camera, renderer, controls, ghost, _t = performance.now() * 0.001;
...
async function init() {
  ...
  // Scene + camera
  scene = new THREE.Scene();
  scene.background = new THREE.Color(0x8fb3d9);            // soft sky
  scene.fog = new THREE.FogExp2(0xDFE9F3, 0.00015);        // mild fog just for depth
  ...
  // Ghost (shader-ready, no animation except emissive “breathing”)
  ghost = new Ghost({
    url: '/assets/models/ghost/ghost.glb',  // make sure this path matches your tree
    targetHeight: 2.2,
    opacityBody: 0.75,
  });
  await ghost.load();
  ghost.setPosition(0, 0.01, 0); // the class lifts base to y=0
  ghost.addTo(scene);

  // For quick console pokes
  window.ghost = ghost;
...
function animate() {
  ...
  if (ghost) ghost.update(dt); // advances uPulseTime
