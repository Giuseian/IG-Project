...
let scene, camera, renderer, controls, ghost;
...
async function init() {
  ...
  controls.target.set(0, 1.2, 0); // guarda leggermente verso il basso
  controls.update();
  ...
  // Ground (abbassato di 10 cm per margine visivo)
  const ground = new THREE.Mesh(
    new THREE.PlaneGeometry(200, 200),
    new THREE.MeshStandardMaterial({ color: 0x6b5b53, roughness: 1 })
  );
  ground.rotation.x = -Math.PI / 2;
  ground.position.y = -0.10;     // <— piccolo offset verso il basso
  scene.add(ground);

  // funzione altezza terreno (piano piatto)
  const getGroundY = (x, z) => ground.position.y;
  ...
  ghost = new Ghost({
    ...
    targetHeight: 2.2,
    opacityBody: 0.75,
    getGroundY,      // clamp rispetto al terreno reale
    clearance: 0.06, // 6 cm di margine sopra il terreno
  });
  await ghost.load();

  // 1) Alza la root (sposta tutto su)
  ghost.setPosition(0, 1.40, 0).addTo(scene);


  ghost.setIdleParams({
    baseY: 0.45,  // era 0.20 → centro del bob più alto
    ampBob: 0.05, // ±5 cm (lascia invariato o cambia a piacere)
    minY:  0.40,  // aggiorna in coerenza
    maxY:  0.50,  // aggiorna in coerenza
  });


  focusOnGhost(4.5); // distanza comoda

  addEventListener('keydown', (e) => {
    const k = e.key.toLowerCase();
    if (k === 'f') focusOnGhost(2.0);
    if (k === '-')  setCamDistance(-0.5);
    if (k === '=' || k === '+') setCamDistance(+0.5);
  });
...
function setCamDistance(delta) {
  const v = new THREE.Vector3().subVectors(camera.position, controls.target);
  const elev = v.y;
  const d = THREE.MathUtils.clamp(v.length() + delta, controls.minDistance, controls.maxDistance);
  if (d < 1e-3) return;
  v.setLength(d); v.y = elev;
