// // // // // // main.js — Pines + FBM Fog (safe) + HUD + Ghost SPAWNER + WASD + BeamSystem (GIMBAL) + SANCTUARIES
...
function makeSanctuarySpots(count, opt = {}) {
  const {
    bands = [[900,1100], [1500,1900], [2300,3000]], // vicino, medio, lontano
    seed  = 1337,
    occluders = [],
    gridCellSize = 120,
    totemRadius = 24,
    margin = 16,
    minSeparation = 280,
    radius = 40,           // raggio del ring visuale
    holdSeconds = 3.0,
    tries = 3000,
    expandStep = 220,
    maxExpansions = 5
  ...
  for (let i=0; i<count; i++){
    ...
    while (!placed && attempts < tries) {
      attempts++;
      // se dopo molti tentativi non trovo, allargo la banda
    ...
    // fallback deterministico: piazza lontano se non trovato
    if (!placed){
      const r  = THREE.MathUtils.lerp(2600, 3400, rng());
      const th = rng() * Math.PI * 2;
      const x  = Math.cos(th) * r;
      const z  = Math.sin(th) * r;
      spots.push({ x, z, radius, holdSeconds });
...
async function init(){
  ...
  // ===== INPUT =====
  // RMB = AIM (non tocca la rotazione FPS; solo gimbal del beam)
  addEventListener('mousedown', (e)=>{
  ...
  const items = makeSanctuarySpots(TOTEM_COUNT, {
    bands: [[900,1100], [1500,1900], [2300,3000]],
    seed: 1337,
