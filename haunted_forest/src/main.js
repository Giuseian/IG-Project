// // // // // // main.js — Pines + FBM Fog + HUD + Spawner + WASD + Beam + Sanctuaries + DPS + Overlays + Wisps + HeavyFog
// NEBBIA MENO MOVIMENTATA CON [, ] FUNZIONANTI + NEBBIA NEGLI SPAZI 
...
// Foschia base più forte
const INIT_FOG_DENSITY   = 4.0e-4;    // <<< prima 1.6e-4
const FOG_DENSITY_MULT   = 1.5;       // <<< boost globale applicato all’avvio/reset
// Movimento/rumore fog molto più calmo
const FOG_TIME_SPEED     = 0.12;      // <<< velocità animazione (0.0 = statica)
const FOG_NOISE_STRENGTH = 0.35;      // <<< 0 = liscio, 1 = noise pieno
// Boost “near” per compattare la foschia a ridosso (aiuta a chiudere i vuoti)
const FOG_NEAR_BOOST     = 0.18;      // <<< 0..~0.4; aumenta copertura a distanze brevi
...
THREE.ShaderChunk.fog_pars_fragment = NOISE_GLSL + `
  ...
  uniform float fogTimeSpeed;   // <<< new
  uniform float fogNoise;       // <<< new (0..1)
  uniform float fogNearBoost;   // <<< new
...
THREE.ShaderChunk.fog_fragment = `
  ...
  n = mix(1.0, n, clamp(fogNoise, 0.0, 1.0));  // 1.0=uniforme, 0.0=rumore spento
  ...
  float heightFactor = 0.14; // leggermente più alto
...
async function init(){
  ...
  scene.fog = new THREE.FogExp2(0xDFE9F3, INIT_FOG_DENSITY * FOG_DENSITY_MULT); // <<< densità più alta
  ...
  // Sky Fog Dome — riempie gli spazi con fog (oltre al background)
  _skyFogDome = createSkyFogDome(scene.fog.color.getHex(), 18000); // <<<
  scene.add(_skyFogDome);
  attachFogTo(_skyFogDome); // <<<
  // Nota: puoi anche mettere scene.background = null per far prevalere il dome.
  ...
  addEventListener('keydown',   (e)=>{
    ...
    // <<< Fog controls: moltiplicativi, Shift = passi grandi
    if (code === 'BracketLeft')  {
    ...
    if (code === 'KeyM') { animateFog = !animateFog; } // toggle movimento
  });
...
function setupDebug(beamTargetObj){
  ...
  addEventListener('keydown', (e)=>{
    switch(e.code){
      ...
      // gli altri controlli fog sono sopra per poter fare preventDefault
      default: break;
    }
  });
...
function updateDebug(spStats = {}){
  ...
  debugEl.innerHTML =
    ...
    `| Beam: ${beamState}  (Pointer Lock: click canvas, ESC | RMB: AIM | F: power | P:spawn, V:antiPopIn, C:cleanse all, X:nearest, ,/. angle, 9/0 range, Q/E snap, Shift+Q 180°, W: wisps, [/] fog, Shift+[|] big step, M: fog anim)`;
}
...
function resetGame(){
  ...
  // --- NUOVO: rimetti la camera/controlli nella posa di spawn
  // (0,20,120) con yaw/pitch neutri. y=null usa ground+eyeHeight.
  playerCtl?.resetPose({ x: 0, z: 120, y: null, yaw: 0, pitch: 0, zeroVel: true });

  // --- NUOVO: reset beam (heat/overheat/aim ecc.)
  beamSystem?.reset?.();
  ...
  // if (scene?.fog) scene.fog.density = INIT_FOG_DENSITY * FOG_DENSITY_MULT; // <<<
  // camera.position.set(0,20,120);
...
// // NEBBIA PIù MOVIMENTATA CON [,] CHE AGISCONO SULLO SHADER 
// import * as THREE from 'https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.module.js';
