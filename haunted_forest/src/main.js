...
// Esempio struttura:
// textures/Grass002/Grass002_4K_Color.jpg
// textures/Grass002/Grass002_4K_NormalGL.jpg
// textures/Grass002/Grass002_4K_Roughness.jpg
// textures/Grass002/Grass002_4K_AmbientOcclusion.jpg
...
const SKY_DAY_TOP    = new THREE.Color(0x89c7ff);
const SKY_DAY_BOTTOM = FOG_DAY.clone();      // match fog color → no banding
const SKY_NIGHT_TOP  = new THREE.Color(0x0a1330);
const SKY_NIGHT_BOTTOM = FOG_NIGHT.clone();  // match fog color → no banding
...
const SKY_FRAG = /* glsl */`
  ...
  void main() {
    ...
    // horizon fog (saturates at the horizon)
    float t = 1.0 - smoothstep(0.0, uHorizonWidth, max(dir.y, 0.0));
...
function init() {
  ...
  // NB: se in futuro userai displacement, aumenta i segmenti: new THREE.PlaneGeometry(200, 200, 256, 256)
  const groundGeo = new THREE.PlaneGeometry(200, 200);
  const groundMat = new THREE.MeshStandardMaterial({
    color: PALETTE.night.groundColor, // tint by palette
    roughness: 0.9,
    metalness: 0.0
  });
  ground = new THREE.Mesh(groundGeo, groundMat);
...
function setupGroundTextures(){
  ...
  // Applica alla MeshStandardMaterial
  ...
  // --- Displacement (opzionale) ---
  // Se vorrai usarlo:
  // const disp = loader.load(TEX_BASE + 'Grass002_4K_Displacement.jpg');
  // disp.wrapS = disp.wrapT = THREE.RepeatWrapping;
  // disp.repeat.set(TILE, TILE);
  // ground.material.displacementMap = disp;
  // ground.material.displacementScale = 0.03; // valori piccoli!
}
...
function updateDayNight(timeSec) {
  ...
  // tint della texture con palette
  // ground.material.color.copy(new THREE.Color(PALETTE.night.groundColor)).lerp(new THREE.Color(PALETTE.day.groundColor), wAmb);
  ground.material.color.set(0xffffff);   // nessuna moltiplicazione → colori reali
...
function updateDebugUI() {
  ...
  debugEl.textContent =
    `fog: ${scene.fog.density.toFixed(3)} | exp: ${renderer.toneMappingExposure.toFixed(2)} | ` +
    `amb: ${ambient.intensity.toFixed(2)} | sun: ${sun.intensity.toFixed(2)} | tile: ${_dbg.tile}\n` +
    `sunUp: ${_dbg.sunUp.toFixed(2)} | wAmb: ${_dbg.wAmb.toFixed(2)} | wExp: ${_dbg.wExp.toFixed(2)} | wFog: ${_dbg.wFog.toFixed(2)}\n` +
    `sky uFogDensity: ${_dbg.uFogD.toFixed(4)} | sky uFogHeight: ${_dbg.uFogH.toFixed(1)}\n` +
    `[N]/[M] palette | time: [P] pause  [U]/[O]/[Y]/[I]/[R] jump | fog: [ ]  - = | ambient 1/2 | sun 3/4 | tiling ,/. | Presets F1/F2/F3`;
}
