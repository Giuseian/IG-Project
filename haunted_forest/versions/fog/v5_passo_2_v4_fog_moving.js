...
// // // // PASSO 2 - Combining with Github + fog starts moving  
...
// 1) aggiungiamo world position al vertex
...
// 3) formula della fog (FIX: preserva segno di dir.y)
THREE.ShaderChunk.fog_fragment = `
  ...
  // Rumore animato (scala e velocità moderate)
  vec3 sampleP = vWorldPosition * 0.00022 + vec3(0.0, 0.0, fogTime * 0.02);
  ...
  float y = dir.y;
  if (abs(y) < 1e-4) y = (y < 0.0 ? -1.0 : 1.0) * 1e-4;

  float heightFactor = 0.055; // look naturale (per test avevamo 0.12)
...
// Hook per abilitare fog + aggiungere uniform fogTime ai materiali
  ...
  root.traverse?.((child)=>{
    ...
    mats.forEach(m=>{
      ...
      m.onBeforeCompile = (shader)=>{
        ...
        console.count('onBeforeCompile fog-patched'); // diagnostica
      };
      m.needsUpdate = true;
    });
  });
...
function init(){
  ...
  // FOG BASELINE (Exp2) — valori naturali
  scene.fog = new THREE.FogExp2(0xDFE3EB, 0.006);
  ...
  // >>> abilita la fog procedurale su tutti i materiali creati
  attachFogTo(scene);
...
function animate(){
  ...
  // anima il rumore della fog
  const t = performance.now() * 0.001;
  _fogShaders.forEach(s => { s.uniforms.fogTime.value = t; });

  controls.update();
  renderer.render(scene, camera);

  // aggiorna overlay OGNI FRAME, così vedi shaders > 0
  updateDebug();
}
...
function updateDebug(){
  if(!debugEl) return;
  debugEl.textContent =
    `FogExp2+FBM density: ${scene.fog?.density.toFixed(3)}  |  ` +
    `Exposure: ${renderer.toneMappingExposure.toFixed(2)}  |  ` +
    `shaders:${_fogShaders.size}\n` +
